...
export class UserService {
  ...
  async validateUser(email: string, password: string) {
    if (!email || !password || email.length > 254 || password.length > 72) {
      return null;
    }
    ...
    const status = compareSync(password, user.password);
    if (status) {
      return user;
    }
    return null;
  ...
  async verifyRefreshToken(refreshToken: string, userId: number) {
    ...
    const status = compareSync(refreshToken, user.refresh_token);
    if (status) {
      return user;
    }
    return false;
